...
void Main()
{
	...
	"----------------".Dump("使用表达式树 1");
	{
		static void Init()
		{
			Func<CreateClaptrapInput, int, ValidateResult> _func;
			...
			PropertyValidationClass._func = expression.Compile();
		}
		Init();
		{
			var input = new CreateClaptrapInput { Name = "wwmin" };
	...
	"----------------".Dump("使用表达式树 2");
	{
		...
		static void Init()
		{
			...
			PropertyValidationClass._func = finalExpression.Compile();
...
{
	public static Func<CreateClaptrapInput, int, ValidateResult> _func;
